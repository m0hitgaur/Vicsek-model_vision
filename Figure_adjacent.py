...
for j in range(0,numberofangles): 
    for i in range(0,numberofnoises):
        adj= np.zeros(numberoftimesteps)
        time=np.zeros(numberoftimesteps)
        for trial in range(0,numberoftrial):
            # Path templates
            path = current_directory+f'/Angle_{angles[j]}/Noise_{noises[i]}/Adjacency_data/adjacency_{trial}_.dat'
            # Load adj parameter vs time data
            o,t = load_data(path)
            adj+=o
            time+=t
        ...
        if(n==0 and m==0):
            ax.plot(time,adj, c=colour[i], marker=markers[i],label=r"$\eta =$"+f"{noises_str[i]}", markeredgecolor=colour[i],markerfacecolor="None")
        else:
            ax.plot(time,adj, c=colour[i], marker=markers[i], markeredgecolor=colour[i],markerfacecolor="None")
        ...
        if(n==0 and m==0):
            ax.annotate( r"$\alpha$ = " + f'{angles_rad[j]}',size=17,xy=(0.6,0.85),xycoords="axes fraction")
        else:
            ax.annotate( r"$\alpha$ = " + f'{angles_rad[j]}',size=17,xy=(0.5,0.1),xycoords="axes fraction")
        ax.annotate(label[j],size=15,xy=(0.05,0.05),xycoords="axes fraction")
        
        if(n==1):
            ax.set_xlabel(r'$t$',fontsize=25)
        if(m==0):
            ax.set_ylabel(r'$\langle adj \rangle$',fontsize=25,labelpad=1)
