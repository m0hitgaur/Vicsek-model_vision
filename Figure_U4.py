...
for j in range(0,numberofangles): 
    for i in range(0,numberofnoises):
        # Path templates
        path = current_directory+f'/Angle_{angles[j]}/Noise_{noises[i]}/Binder_Cumulant/U4_vs_t.dat'
        # Load order parameter vs time data
        u4,time = load_data(path)
        ...
        if(n==0 and m==1):
            ax.plot(time,u4, c=colour[i], marker=markers[i],label=r"$\eta =$"+f"{noises_str[i]}", markeredgecolor=colour[i],markerfacecolor="None")
        else:
            ax.plot(time,u4, c=colour[i], marker=markers[i], markeredgecolor=colour[i],markerfacecolor="None")
        ...
        ax.yaxis.set_minor_locator(MultipleLocator(0.008))
        ax.tick_params(which='minor', direction="in", length=4, color="black",right=True,top=True)
        
        if(n==1 and m==0 ):
            ax.annotate( r"$\alpha$ = " + f'{angles_rad[j]}',size=17,xy=(0.5,0.87),xycoords="axes fraction")
        else:
            ax.annotate( r"$\alpha$ = " + f'{angles_rad[j]}',size=17,xy=(0.6,0.87),xycoords="axes fraction")
        ax.annotate(label[j],size=15,xy=(0.05,0.85),xycoords="axes fraction")
        
        if(n==1):
            ax.set_xlabel(r'$t$',fontsize=25)
        if(m==0):
            ax.set_ylabel(r'$U_{4} $',fontsize=25,labelpad=1)
        
        ax.legend(loc="best", prop={'size': 15},labelspacing = 0.2,frameon=False,bbox_to_anchor=(0.046, 0.4, 0.5, 0.5))
