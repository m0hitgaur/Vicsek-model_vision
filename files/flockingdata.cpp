...
double orderparameter(vector <vector<double>>& position,vector<double>& theta,int Number_of_active_agents,double half_angle,double radius,double noise,double velocity,double dt,double Lx,double Ly,int time ,int trial)
  {
    ...
    double avgx=0.0,avgy=0.0,v=0.0 ;
    ...
    avgx/=Number_of_active_agents ;
    avgy/=Number_of_active_agents ;
    v=sqrt((avgx*avgx) + (avgy*avgy));
    return v;
...
int main() {
    ...
    {   trial_time=time(NULL);
        ...
        //update theta implemented from paper 
            vector<double> avgx(Number_of_active_agents,0);
            vector<double> avgy(Number_of_active_agents,0);
            vector<int> count(Number_of_active_agents,1); // starting from 1 as the particle itself is always counted
            
            for (int i = 0; i < Number_of_active_agents; i++) {
                avgx[i]+=cos(theta[i]);   // just so that we can include the particle itself in average
                avgy[i]+=sin(theta[i]);   // just so that we can include the particle itself in average
