...
ax_in=inset_axes(axes, width="80%", height="80%", loc="lower right",bbox_to_anchor=(0.3, .1, .7, .6),bbox_transform=axes.transAxes)
...
path_corr_45 = current_directory+f'/Angle_{angles[0]}/Noise_{noises[0]}/correlation_data/connectedcorrlength_vs_time_zerocrossing.dat'
path_corr_180 = current_directory+f'/Angle_{angles[1]}/Noise_{noises[0]}/correlation_data/connectedcorrlength_vs_time_zerocrossing.dat'

path_velo_corr_45 = current_directory+f'/Angle_{angles[0]}/Noise_{noises[0]}/correlation_data/velocorrlength_vs_time_zerocrossing.dat'
path_velo_corr_180 = current_directory+f'/Angle_{angles[1]}/Noise_{noises[0]}/correlation_data/velocorrlength_vs_time_zerocrossing.dat'
# Load order parameter vs time data
corr_45,t_corr_45 = load_data(path_corr_45)
corr_180,t_corr_180 = load_data(path_corr_180)
velo_corr_45,velo_t_corr_45 = load_data(path_velo_corr_45)
velo_corr_180,velo_t_corr_180 = load_data(path_velo_corr_180)
delete=[]
for i in range(len(corr_45)-1):
    if t_corr_45[i]>1000:
        if(i%2==0):
            delete.append(i)
            print(i)
print(delete)
corr_45,t_corr_45 = np.delete(corr_45,delete),np.delete(t_corr_45,delete)
corr_180,t_corr_180 = np.delete(corr_180,delete),np.delete(t_corr_180,delete)

velo_corr_45,velo_t_corr_45 = np.delete(velo_corr_45,delete),np.delete(velo_t_corr_45,delete)
velo_corr_180,velo_t_corr_180 = np.delete(velo_corr_180,delete),np.delete(velo_t_corr_180,delete)
...
#for ticks 
ax_in.xaxis.set_major_locator(MultipleLocator(1000))
ax_in.xaxis.set_minor_locator(MultipleLocator(500))
ax_in.yaxis.set_major_locator(MultipleLocator(0.5))
ax_in.yaxis.set_minor_locator(MultipleLocator(0.1))
ax_in.tick_params(which='minor', direction="in", length=4, color="black",right=True,top=True)
...
#ax.set_ylabel(r'$\langle \xi_{\delta v} \rangle$',fontsize=25)
ax.annotate(r'$\langle \xi_{\delta v} \rangle$',size=25,xy=(-0.14,0.4),xycoords="axes fraction",rotation=90)

ax.annotate(r"$\langle \xi_v \rangle$",size=20,xy=(0.3,0.4),xycoords="axes fraction",rotation=90)
ax.annotate(r"$t$",size=20,xy=(0.95,0.05),xycoords="axes fraction")
ax.legend(loc="best", prop={'size': 12},labelspacing = 0.2,frameon=False,bbox_to_anchor=(0.01, 0.1, 0.5, 0.5))
...
ax_in.legend(loc="best", prop={'size': 12},labelspacing = 0.2,frameon=False,bbox_to_anchor=(0.2, 0.08, 0.5, 0.5))
ax_in.title.set_position((0.15, 0.9))
#ax_in.set_ylim(0,6)
ax_in.set_xlim(1,5000)
ax_in.tick_params(axis='x', which='minor')
ax_in.tick_params(left=True, top=True ,right=True)
ax_in.tick_params(axis="x", direction="in")
ax_in.tick_params(axis="y", direction="in")
...
ax.text(0.1,0.9,string,transform=ax.transAxes,fontsize=13,verticalalignment='top',bbox=props)
