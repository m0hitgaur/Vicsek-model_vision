...
angles_rad=["$\pi/4$","$\pi/2$","$2\pi/3$","$\pi$"]
noises_str=[r"$2.0$",r"$0.5$",r"$0.05$"]
...
ax_noise=[[0,0],[1,0],[2,0]]
ax_angle=[[0,0],[0,1],[0,2],[0,3]]

for ax in ax_noise:
    n=ax[0]
    m=ax[1]
    ax1=axes[n,m]
    ax1.annotate( r"$\eta=$"+f"{noises_str[n]}",size=35,xy=(-0.2,0.2),xycoords="axes fraction",rotation=90)    

for ax in ax_angle:
    n=ax[0]
    m=ax[1]
    ax1=axes[n,m]
    ax1.annotate( r"$\alpha=$"+f"{angles_rad[m]}",size=35,xy=(0.35,1.1),xycoords="axes fraction")    
