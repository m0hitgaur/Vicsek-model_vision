...
for j in range(0,numberofangles): 
    for i in range(0,numberofnoises):
        # Path templates
        path = current_directory+f'/Angle_{angles[j]}/Noise_{noises[i]}/Avg_nearest_neighbour_data/avg_near_neigh.dat'
        # Load order parameter vs time data
        u2,time = load_data(path)
        ...
        if(n==0 and m==0):
            ax.plot(time,u2, c=colour[i], marker=markers[i],label=r"$\eta =$"+f"{noises_str[i]}", markeredgecolor=colour[i],markerfacecolor="None")
        else:
            ax.plot(time,u2, c=colour[i], marker=markers[i], markeredgecolor=colour[i],markerfacecolor="None")
        ...
        if( m==0):
            ax.annotate( r"$\alpha$ = " + f'{angles_rad[j]}',size=17,xy=(0.54,0.87),xycoords="axes fraction")
        else:
            ax.annotate( r"$\alpha$ = " + f'{angles_rad[j]}',size=17,xy=(0.45,0.87),xycoords="axes fraction")
        ax.annotate(label[j],size=15,xy=(0.05,0.85),xycoords="axes fraction")
        
        if(n==1):
            ax.set_xlabel(r'$t$',fontsize=25)
        if(m==0):
            ax.set_ylabel(r'$\langle n \rangle $',fontsize=25,labelpad=1)
        
        ax.legend(loc="best", prop={'size': 15},labelspacing = 0.2,frameon=False,bbox_to_anchor=(0.046, 0.05, 0.5, 0.5))
