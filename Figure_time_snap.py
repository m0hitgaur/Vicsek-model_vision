...
# Parameters
time=[50,100,1000,5000]#input("timee to plot")  # 2500  # Adjust this for different time steps
trial = 24
noises = [0.05]
angles = [45,180]
...
for i,ax in enumerate(ax_time):
    ...
    m=ax[1]
    ax1=axes[n,m]
    ax1.annotate( r"$t="+f"{time[m]}"+r"$",size=35,xy=(0.35,1.1),xycoords="axes fraction")    

for j,ax in enumerate(ax_angle):
    n=ax[0]
    m=ax[1]
    ax1=axes[n,m]
    ax1.annotate( r"$\alpha=$"+f"{angles_rad[n]}",size=40,xy=(-0.2,0.2),xycoords="axes fraction",rotation=90)    


#horizontal 1  (left=>right)
line = plt.Line2D([0.035, 0.86], [0.483, 0.483], transform=fig.transFigure, color="black", linewidth=2)
fig.add_artist(line)


#vertical 5
line = plt.Line2D([0.86, 0.86], [0.07, 0.9], transform=fig.transFigure, color="black", linewidth=2)
fig.add_artist(line)

#vertical 5
line = plt.Line2D([0.035, 0.035], [0.07, 0.9], transform=fig.transFigure, color="black", linewidth=2)
fig.add_artist(line)


#horizontal 4 (top=>bottom)
line = plt.Line2D([0.035, 0.86], [0.07, 0.07], transform=fig.transFigure, color="black", linewidth=2)
fig.add_artist(line)

#horizontal 1
line = plt.Line2D([0.035, 0.86], [0.9, 0.9], transform=fig.transFigure, color="black", linewidth=2)
fig.add_artist(line)


plt.subplots_adjust(left=0.043, right=0.852, top=0.88, bottom=0.086, wspace=0.06, hspace=0.13)
