...
for j in range(0,numberofangles): 
    for i in range(0,numberofnoises):
        ...
        path = current_directory+f'/Angle_{angles[j]}/Noise_{noises[i]}/correlation_data/connectedcorrlength_vs_time_zerocrossing.dat'
        # Load order parameter vs time data
        ccf,t = load_data(path)
        ...
        if(n==0 and m==0):
            ax.plot(t,ccf, c=colour[i], marker=markers[i],label=r"$\eta =$"+f"{noises_str[i]}", markeredgecolor=colour[i],markerfacecolor="None")
        else:
            ax.plot(t,ccf, c=colour[i], marker=markers[i], markeredgecolor=colour[i],markerfacecolor="None")
        ...
        if(m==0):
            ax.set_ylabel(r'$\xi_{v} $',fontsize=25,labelpad=1)
        
        ax.legend(loc="best", prop={'size': 12},labelspacing = 0.2,frameon=False,bbox_to_anchor=(0.5, 0.25, 0.5, 0.5))
        ax.title.set_position((0.15, 0.9))
        #if(n==0 and m==0):ax.set_ylim(0,6)
        #else: ax.set_ylim(2,6)
        ax.set_xlim(0,5000)
