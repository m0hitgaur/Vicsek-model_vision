...
for i in range(len(time)):
    path = current_directory+f'/Angle_{angles[0]}/Noise_{noises[0]}/correlation_data/velocitycorrelation_vs_r_{time[i]}_.dat'
    if(i==3):
        path = current_directory+f'/Figure/correlation/vcf_vs_r_{int(angles[0])}_{noise_str[0]}.dat'
    # Load connected correlation function vs r data
    vcf,r = load_data(path)
    ax=axes[0,0]
    ax.plot(r,vcf,c=colour_time[i], marker=markers[i],markerfacecolor="none", markeredgecolor=colour_time[i],label="$t=$"+f"{time[i]}")
    ...
    ax.plot(r,vcf,c=colour_time[i], marker=markers[i],markerfacecolor="none")
    ...
    ax.annotate(r'$C_{ v}(r)$',size=30,xy=(-0.3,0.4),xycoords="axes fraction",rotation=90)
    
    #ax.set_ylabel(r'$C_{ v}(r)$',fontsize=30)
...
for i in range(len(time)):
    path = current_directory+f'/Angle_{int(angles[0])}/Noise_{noises[1]}/correlation_data/velocitycorrelation_vs_r_{time[i]}_.dat'
    if(i==3):
        path = current_directory+f'/Figure/correlation/vcf_vs_r_{int(angles[0])}_{noise_str[1]}.dat'
    # Load connected correlation function vs r data
    vcf,r = load_data(path)
    ax=axes[1,0]
    #ax.set_xlabel('$r$',fontsize=25)
...
for i in range(len(time)):
    path = current_directory+f'/Angle_{angles[1]}/Noise_{noises[0]}/correlation_data/velocitycorrelation_vs_r_{time[i]}_.dat'
    #if(i==3):
...
for i in range(len(time)):
    path = current_directory+f'/Angle_{angles[1]}/Noise_{noises[1]}/correlation_data/velocitycorrelation_vs_r_{time[i]}_.dat'
    #if(i==3):
    ...
    ax.tick_params(which='major', direction="in", length=6, color="black",right=True,top=True,left=True,bottom=True)
plt.tight_layout()
plt.savefig('fig_vcf_time.pdf', format='pdf',bbox_inches='tight')
