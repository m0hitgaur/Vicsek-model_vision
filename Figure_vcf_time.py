...
for i in range(len(time)):
    ...
    ax.set_ylabel(r'$C_{v}(r)$',fontsize=35)
    ...
    ax.tick_params(which='major', direction="in", length=8, color="black",right=True,top=True,left=True,bottom=True)



for i in range(len(time)):
    path = current_directory+f'/Angle_{angles[0]}/Noise_{noises[1]}/correlation_data/velocitycorrelation_vs_r_{time[i]}_.dat'
    if(i==3):
    ...
    ax.annotate(label[1],size=label_size,xy=(0.06,0.05),xycoords="axes fraction")
